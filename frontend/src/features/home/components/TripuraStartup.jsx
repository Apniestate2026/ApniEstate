...
import startupIndiaLogo from "../../../assets/startup-india-logo.webp";
...
const startupIndia = {
  ...
  glowColor: "from-violet-200 via-purple-200 to-fuchsia-200",
...
const CredentialCard = ({ cred, className = "" }) => (
  <motion.div
  ...
  return (
    <div className="pt-2 pb-4 md:pt-3 md:pb-6">
      <div className="max-w-5xl mx-auto px-2 sm:px-6 lg:px-8">
        ...
        >
          {/* Startup India — standalone top row */}
          <div className="w-[calc(50%-0.25rem)] sm:w-[calc(50%-0.5rem)] md:w-[calc(50%-0.75rem)]">
            <CredentialCard cred={startupIndia} />
          </div>

          {/* Startup Tripura + DIT — bottom row */}
          <div className="flex flex-row items-stretch justify-center gap-2 sm:gap-4 md:gap-6 w-full">
            {bottomCards.map((cred, idx) => (
              <CredentialCard key={idx} cred={cred} className="flex-1 min-w-0" />
            ))}
          </div>
        </motion.div>
      </div>
